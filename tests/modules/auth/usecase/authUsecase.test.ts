...
const createMockRepo = (overrides: Partial<Record<keyof AuthRepository, any>> = {}): AuthRepository => {
  const repo: any = {
...
describe("AuthUsecase", () => {
  describe("login", () => {
    it("should login successfully and return access token", async () => {
      ...
      const usecase = new AuthUsecaseImpl(repo);
    ...
    it("should throw UnauthorizedError when user not found", async () => {
      ...
      await expect(
        usecase.login({ email: "unknown@example.com", password: "secret123" }),
    ...
    it("should throw UnauthorizedError when password is wrong", async () => {
      ...
      await expect(
        usecase.login({ email: "user@example.com", password: "wrongpassword" }),
      ).rejects.toThrow(UnauthorizedError);
    });
  });
  ...
  describe("me", () => {
    it("should return user profile when found", async () => {
      ...
      const usecase = new AuthUsecaseImpl(repo);
      const result = await usecase.me("1");
    ...
    it("should throw NotFoundError when user not found", async () => {
      ...
      await expect(usecase.me("nonexistent")).rejects.toThrow(NotFoundError);
    });
  });
});
