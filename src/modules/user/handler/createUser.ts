...
import { createUser } from "@/modules/user/usecase/createUser";
...
export const createUserHandler = async (c: Context<AppHonoEnv>) => {
  ...
  const result = await createUser(parsed.data, logger);
